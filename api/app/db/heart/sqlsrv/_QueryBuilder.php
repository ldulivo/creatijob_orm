...
class _QueryBuilder extends _Where
{
     ...
     * -----
     * Description:
     * Stores the limit value to be applied via OFFSET-FETCH clause in SQL Server.
     * 
     ...
     * -----
     * Description:
     * Stores the offset value to be applied via OFFSET-FETCH clause in SQL Server.
     * 
    ...
    public function get()
    {
        $sql = "SELECT $this->columns FROM $this->table " . $this->buildWhere();

        if (!empty($this->orderBy)) {
            $sql .= " $this->orderBy";
        } else {
            // Se requiere ORDER BY si se usa OFFSET en SQL Server
        ...
        if ($this->offset !== '' || $this->limit !== '') {
            ...
            $limit = $this->limit !== '' ? (int) $this->limit : 10; // valor por defecto si no se definió limit
     ...
     * @return int|null The number of rows affected by the update, or null if none.
     * -----
     */
     ...
     * It logs any errors encountered during the execution of the query.
     * 
     * @return int|false The number of rows affected by the deletion, or false if no match.
     * -----
     */
